    ...
    internal partial class CoinExSocketClientSpotApi : ICoinExSocketClientSpotApiShared
    {
        ...
        async Task<ExchangeResult<UpdateSubscription>> ITradeSocketClient.SubscribeToTradeUpdatesAsync(SubscribeTradeRequest request, Action<DataEvent<SharedTrade[]>> handler, CancellationToken ct)
        {
            ...
            var result = await SubscribeToTradeUpdatesAsync(symbols, update =>
            {
                ...
                handler(update.ToType<SharedTrade[]>(update.Data.Select(x => new SharedTrade(ExchangeSymbolCache.ParseSymbol(_topicId, update.Symbol), update.Symbol!, x.Quantity, x.Price, x.Timestamp)
                {
